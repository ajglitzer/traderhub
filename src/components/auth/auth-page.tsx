...
export function AuthPage({ onAuth }: { onAuth: () => void }) {
  ...
  // Surface a failed OAuth round-trip (e.g. the /auth/callback code exchange
  // failed) instead of silently dropping the user back on the login form.
  useEffect(() => {
  ...
  const signInWithGoogle = async () => {
    ...
    try {
      ...
      // there's nothing else to do here; only reachable on failure.
          ...
          {mode !== "forgot" && (
            <>
              <button onClick={signInWithGoogle} disabled={googleLoading} style={{
                ...
                background:"#fff", color:"#1f1f1f", fontSize:13, fontWeight:700,
                cursor: googleLoading ? "default" : "pointer", marginBottom:16,
                display:"flex", alignItems:"center", justifyContent:"center", gap:10,
                opacity: googleLoading ? 0.7 : 1,
              }}>
                {googleLoading
                  ? <div style={{ width:16, height:16, borderRadius:"50%", border:"2px solid rgba(0,0,0,0.15)", borderTop:"2px solid #1f1f1f", animation:"spin 0.8s linear infinite" }}/>
                  : <GoogleIcon/>
                }
              ...
              <div style={{ display:"flex", alignItems:"center", gap:10, marginBottom:16 }}>
                ...
                <span style={{ fontSize:10, color:"#4b5563", textTransform:"uppercase" as const, letterSpacing:"0.06em" }}>or</span>
                <div style={{ flex:1, height:1, background:"rgba(255,255,255,0.08)" }}/>
              </div>
            </>
          )}
